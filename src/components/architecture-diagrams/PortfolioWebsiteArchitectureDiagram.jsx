...
import { Monitor, Smartphone, Cloud, Code, Package, Mail, Github, Globe, Palette, Zap } from 'lucide-react';
  ...
  return (
    <div className="w-full overflow-x-auto">
      <div className="min-w-[900px] p-4 md:p-8 bg-gray-900 rounded-lg border border-gray-700">
        ...
        {/* Data Flow Legend */}
        <div className="mt-6 md:mt-8 p-3 md:p-4 bg-gray-800/50 rounded-lg">
...
const ServiceBox = ({ icon: Icon, title, subtitle, features, color }) => {
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col items-center p-3 md:p-4 rounded-lg bg-${color}-900/20 border border-${color}-500/30 min-w-[140px] md:min-w-[180px]`}
    >
      ...
      <div className="text-[10px] md:text-xs text-gray-400 mb-2 text-center">{subtitle}</div>
      <div className="space-y-1">
        {features.map((feature, idx) => (
          <div key={idx} className="text-[10px] md:text-xs text-gray-300 text-center">
            • {feature}
          </div>
        ))}
      </div>
...
const ComponentBox = ({ title, subtitle }) => {
  return (
    <div className="flex flex-col items-center justify-center p-2 md:p-3 rounded-lg bg-indigo-900/20 border border-indigo-500/30">
      ...
      <div className="text-[10px] md:text-xs text-gray-400 text-center">{subtitle}</div>
    </div>
  );
};
...
const PipelineStep = ({ title, subtitle }) => {
  return (
    <div className="flex flex-col items-center justify-center p-2 md:p-3 rounded-lg bg-green-900/20 border border-green-500/30 min-w-[80px] md:min-w-[100px]">
      <div className="text-xs md:text-sm font-semibold text-white text-center">{title}</div>
      <div className="text-[10px] md:text-xs text-gray-400 text-center">{subtitle}</div>
    </div>
  );
...
const ArrowDown = ({ label }) => {
  return (
    <div className="flex flex-col items-center justify-center my-2 md:my-3">
      ...
      {label && (
        <span className="text-[10px] md:text-xs text-gray-400 mt-1">{label}</span>
      )}
    </div>
  );
};
